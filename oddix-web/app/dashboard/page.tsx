  ...
  return (
    <section style={styles.heroCard}>
      ...
      <div style={styles.heroVisual}>
        ...
        <div style={styles.heroPlayerHalo} />
...
function PlayerPropCard({ prop, index }: { prop: any; index: number }) {
  ...
  const fallbackPlayer = index === 1 ? "Principal finalizador" : index === 2 ? "Meia ofensivo" : "Atacante destaque";
    ...
    <button style={styles.playerCard}>
      <div style={styles.playerCardTop}>
        ...
        <div style={styles.playerAvatarPremium}>
          <span style={styles.playerPositionLabel}>{index === 1 ? "MEI" : index === 2 ? "ATA" : "ATA"}</span>
          <strong>{prop?.playerInitials || initials(teamName)}</strong>
        </div>
        <TrendChart />
...
const globalCss = `
  * { box-sizing: border-box; }
  ...
  .hero-boost-grid { grid-template-columns: minmax(0, 1fr) 280px; }
  ...
  @media (max-width: 980px) {
    .oddix-rebuild header { grid-template-columns: 1fr !important; }
  }
  @media (max-width: 760px) {
...
const styles: Record<string, CSSProperties> = {
  ...
  header: {
    ...
    alignItems: "center",
    gap: 22,
    background: "linear-gradient(180deg, rgba(7,7,13,.98), rgba(7,7,13,.88))",
    borderBottom: "1px solid rgba(247,201,72,.18)",
  ...
  headerLogo: { maxWidth: 165, maxHeight: 38, objectFit: "contain" },
  ...
  heroCard: {
    ...
    background: "radial-gradient(circle at 68% 44%, rgba(123,44,255,.62), transparent 36%), radial-gradient(circle at 20% 18%, rgba(247,201,72,.10), transparent 26%), linear-gradient(135deg, #10051e 0%, #1A0836 46%, #080711 100%)",
    boxShadow: "0 0 54px rgba(123,44,255,.30), inset 0 1px 0 rgba(255,255,255,.08)",
    display: "grid",
    gridTemplateColumns: "minmax(390px, 49%) minmax(0, 51%)",
    alignItems: "stretch",
    isolation: "isolate",
  },
  heroContent: { position: "relative", zIndex: 5, padding: "34px 0 64px 36px", maxWidth: 560 },
  heroBadge: {
    display: "inline-flex",
  ...
  heroTitle: {
    margin: "8px 0 12px",
    color: "#fff",
    fontSize: "clamp(54px, 5.8vw, 78px)",
    lineHeight: 0.92,
    letterSpacing: -3,
    fontWeight: 1000,
  ...
  heroMainLogo: {
    ...
    height: "auto",
    objectFit: "contain",
    filter: "drop-shadow(0 0 28px rgba(123,44,255,.74)) drop-shadow(0 0 12px rgba(247,201,72,.22))",
  },
  heroVersionBadge: {
    position: "absolute",
    right: -16,
  ...
  heroText: { maxWidth: 500, margin: 0, color: "rgba(255,255,255,.94)", fontSize: 15, lineHeight: 1.35, fontWeight: 800 },
  heroFeatures: { display: "flex", flexWrap: "wrap", gap: 9, marginTop: 16, fontWeight: 900, color: "rgba(255,255,255,.92)", fontSize: 13 },
  heroMetrics: { display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 10, marginTop: 18, maxWidth: 460 },
  heroMetric: {
  ...
  heroVisual: { position: "relative", zIndex: 2, minHeight: 500, overflow: "hidden" },
  energyOrb: {
    position: "absolute",
    width: 620,
    height: 620,
  ...
  heroPlayerHalo: {
    ...
    height: 360,
    borderRadius: 999,
    background: "radial-gradient(circle, rgba(247,201,72,.18), rgba(123,44,255,.32) 42%, transparent 70%)",
    filter: "blur(8px)",
    zIndex: 3,
  },
  heroLogoGhost: {
  ...
  heroPlayer: {
    ...
    filter: "drop-shadow(0 0 46px rgba(123,44,255,.68)) drop-shadow(0 24px 36px rgba(0,0,0,.52))",
    zIndex: 5,
  },
  smartphoneMock: {
    position: "absolute",
  ...
  phoneLine: { height: 7, width: "90%", marginTop: 12, borderRadius: 999, background: "linear-gradient(90deg, #22c55e, #F7C948)" },
  heroSlogan: { position: "absolute", right: 78, bottom: 58, fontSize: 12, fontWeight: 1000, textAlign: "center", color: "rgba(255,255,255,.94)", textShadow: "0 0 18px rgba(123,44,255,.55)", zIndex: 6 },
  heroBottomBar: {
    position: "absolute",
    left: 22,
    right: 22,
  ...
  boostTitle: { color: "#d8b4fe", fontSize: 19, fontWeight: 950, marginBottom: 20 },
  boostOdd: { color: "#F7C948", fontSize: 56, lineHeight: 1, fontWeight: 1000, marginBottom: 10 },
  ...
  topPickCard: {
    ...
    minHeight: 188,
    borderRadius: 28,
    border: "1px solid rgba(247,201,72,.60)",
    background: "radial-gradient(circle at 50% 0%, rgba(247,201,72,.12), transparent 30%), linear-gradient(135deg, rgba(7,7,13,.98), rgba(45,17,77,.76))",
    boxShadow: "0 0 40px rgba(247,201,72,.12), inset 0 1px 0 rgba(255,255,255,.08)",
    padding: 24,
    display: "grid",
    gridTemplateColumns: "125px 320px minmax(270px, 1fr) 430px",
    gap: 22,
    alignItems: "center",
  },
  topPickLabel: { display: "flex", alignItems: "center", gap: 12, color: "#F7C948", fontSize: 16, fontWeight: 1000 },
  topPickTeams: { display: "grid", gridTemplateColumns: "88px minmax(0, 1fr) 88px", gap: 14, alignItems: "center" },
  topPickLogo: { width: 88, height: 88, objectFit: "contain", filter: "drop-shadow(0 0 16px rgba(255,255,255,.28))" },
  topPickMatchText: { display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", fontWeight: 950 },
  topPickMarket: { border: "1px solid rgba(247,201,72,.42)", borderRadius: 20, background: "linear-gradient(135deg, rgba(247,201,72,.11), rgba(255,255,255,.06))", padding: 20, minHeight: 118, display: "flex", flexDirection: "column", justifyContent: "center" },
  topPickStats: { display: "grid", gridTemplateColumns: "76px 94px 100px 142px", gap: 10, alignItems: "stretch" },
  metricBox: { border: "1px solid rgba(255,255,255,.13)", borderRadius: 16, background: "rgba(255,255,255,.07)", padding: 12, display: "flex", flexDirection: "column", justifyContent: "center", fontWeight: 950, minHeight: 68 },
  ...
  playerCard: { position: "relative", overflow: "hidden", minHeight: 305, border: "1px solid rgba(255,255,255,.13)", borderRadius: 22, background: "linear-gradient(160deg, rgba(123,44,255,.72), rgba(45,17,77,.96) 60%, rgba(7,7,13,.96))", color: "#fff", textAlign: "left", cursor: "pointer", boxShadow: "inset 0 1px 0 rgba(255,255,255,.18), 0 18px 36px rgba(0,0,0,.28)" },
  playerCardTop: { position: "relative", height: 158, padding: 18, background: "radial-gradient(circle at 52% 22%, rgba(255,255,255,.18), transparent 32%), linear-gradient(90deg, rgba(255,255,255,.12), rgba(123,44,255,.18))" },
  playerTeamLogo: { width: 82, height: 82, objectFit: "contain", filter: "drop-shadow(0 0 16px rgba(255,255,255,.30))" },
  playerAvatarPremium: { position: "absolute", left: "38%", top: 12, width: 132, height: 132, borderRadius: 30, background: "radial-gradient(circle at 50% 18%, rgba(255,255,255,.22), transparent 34%), linear-gradient(135deg, #F7C948, #7B2CFF 55%, #12051F)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: "#050510", fontSize: 34, fontWeight: 1000, boxShadow: "0 24px 46px rgba(0,0,0,.48), 0 0 28px rgba(247,201,72,.22)", overflow: "hidden" },
  playerPositionLabel: {
    position: "absolute",
    top: 13,
    left: 13,
  ...
  trendChart: { position: "absolute", right: 18, bottom: 14, display: "flex", alignItems: "flex-end", gap: 4 },
  ...
  playerCardBody: { padding: "20px 20px 12px" },
  playerSelectionBox: { marginTop: 10, border: "1px solid rgba(247,201,72,.28)", borderRadius: 14, padding: 13, background: "rgba(7,7,13,.55)", display: "flex", flexDirection: "column", gap: 4 },
  ...
  ticketSection: { display: "grid", gridTemplateColumns: "minmax(0, 1fr) 280px", minHeight: 300, borderRadius: 26, border: "1px solid rgba(247,201,72,.48)", background: "linear-gradient(135deg, rgba(7,7,13,.98), rgba(45,17,77,.65))", overflow: "hidden" },
  ...
  ticketBigOdd: { color: "#F7C948", fontSize: 48, lineHeight: 1, fontWeight: 1000, textShadow: "0 0 24px rgba(247,201,72,.34)" },
  ...
  performanceCard: { minHeight: 122, border: "1px solid rgba(255,255,255,.12)", borderRadius: 18, padding: 18, background: "rgba(255,255,255,.05)", position: "relative", overflow: "hidden" },
