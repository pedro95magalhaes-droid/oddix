...
export class OddixWorldCupResolverService {
  ...
  async resolveToday(question: string): Promise<OddixWorldCupResolution> {
    ...
    this.logger.log(
      `[V17_WORLDCUP] today=${todayIso} local=${localFixtures.length} web=${webFixtures.length} items=${items.length}`,
    );
  ...
  private async getLocalCupFixtures(todayIso: string): Promise<OddixWorldCupFixture[]> {
    ...
    for (const entry of calls) {
      try {
        ...
        if (list.length) {
          this.logger.log(`[V17_WORLDCUP] local.${entry.name}=${list.length}`);
          fixtures.push(...list);
        }
      } catch (error: any) {
        this.logger.warn(`[V17_WORLDCUP] local.${entry.name} falhou: ${error?.message || error}`);
      }
    }
  ...
  private async runWorldCupResearch(question: string, todayIso: string, todayHuman: string) {
    ...
    for (const query of queries) {
      try {
        ...
        if (this.uniqueItems(items).length >= 18) break;
      } catch (error: any) {
    ...
    return {
      items: this.uniqueItems(items).slice(0, 30),
      queries,
      provider,
  ...
  private buildWorldCupQueries(question: string, todayIso: string, todayHuman: string) {
    ...
    const md = this.monthDayEnglish(todayIso);
    const ptDate = todayHuman;

    return Array.from(
      new Set([
        `FIFA World Cup fixtures today ${todayIso} football`,
        `FIFA World Cup matches ${md} ${year} schedule`,
        `Club World Cup matches today ${todayIso} football`,
        `FIFA Club World Cup ${md} ${year} fixtures`,
        `World Cup football fixtures ${todayIso} FlashScore`,
        `ESPN World Cup fixtures ${todayIso}`,
        `Sofascore World Cup matches today ${todayIso}`,
  ...
  private async extractFixturesFromResearch(
    ...
    const compactItems = items.slice(0, 18).map((item) => ({
      title: item.title,
...
${JSON.stringify(compactItems, null, 2)}`,
  ...
  private extractFixturesDeterministically(items: ResearchItem[], todayIso: string): OddixWorldCupFixture[] {
    ...
    const dateNeedles = this.dateNeedles(todayIso);

    for (const item of items) {
      const text = `${item.title} ${item.description}`;
      const normalized = this.normalize(text);
      const hasToday = dateNeedles.some((needle) => normalized.includes(this.normalize(needle))) || normalized.includes('today') || normalized.includes('hoje');
      const hasCup = ['world cup', 'club world cup', 'fifa', 'copa do mundo', 'mundial'].some((term) => normalized.includes(this.normalize(term)));
      if (!hasCup || !hasToday) continue;

      const patterns = [
        /([A-ZÀ-ÿ][A-Za-zÀ-ÿ .'-]{2,40})\s+(?:vs\.?|v\.?|x|contra)\s+([A-ZÀ-ÿ][A-Za-zÀ-ÿ .'-]{2,40})/g,
        /([A-ZÀ-ÿ][A-Za-zÀ-ÿ .'-]{2,40})\s+[-–—]\s+([A-ZÀ-ÿ][A-Za-zÀ-ÿ .'-]{2,40})/g,
      ];
      ...
      for (const pattern of patterns) {
        ...
        while ((match = pattern.exec(text))) {
          ...
          if (!home || !away || home.length > 35 || away.length > 35) continue;
          fixtures.push({
            home,
            away,
