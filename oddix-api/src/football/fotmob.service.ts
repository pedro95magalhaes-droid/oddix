...
export class FootballService {
  ...
  private liveCacheSeconds() {
    return Number(
      process.env.ODDIX_LIVE_CACHE_SECONDS ||
        process.env.FOOTBALL_LIVE_CACHE_SECONDS ||
        120,
    );
  }
  ...
  private fixturesCacheMinutes() {
    return Number(
      process.env.ODDIX_FIXTURES_CACHE_MINUTES ||
        process.env.FOOTBALL_FIXTURES_CACHE_MINUTES ||
        30,
    );
  }

  private minFixturesBeforeFallback() {
    return Number(process.env.ODDIX_MIN_FIXTURES_BEFORE_FALLBACK || 6);
  }

  private shouldUseFotmobForFixtures() {
    return process.env.FOTMOB_USE_FOR_FIXTURES === 'true';
  }

  private shouldUseFotmobForLive() {
    return process.env.FOTMOB_USE_FOR_LIVE === 'true';
  }

  private shouldUseSportScore6Fallback() {
    return process.env.SPORTSCORE6_ENABLE_FALLBACK !== 'false';
  }

  private shouldUseSportsDbFallback() {
    return process.env.THESPORTSDB_ENABLE_FALLBACK === 'true';
  }

  private shouldUseSportmonksFallback() {
    return process.env.SPORTMONKS_ENABLE_FALLBACK === 'true';
  }

  private shouldUseFootballDataFallback() {
    return process.env.FOOTBALL_DATA_ENABLE_FALLBACK === 'true';
  }
  ...
  async getFixtures(date?: string) {
    ...
    /**
     * Economia de API:
     * 1) usa cache fresco primeiro;
     * 2) busca FlashScore + AllScores como fontes principais de agenda;
     * 3) só aciona FotMob/SportScore6/outros se faltar volume;
     * 4) API-Football continua desligada salvo fallback explícito.
     */
    ...
    const searchDates = Array.from(
      new Set(
        Array.from({ length: futureDays + 1 }, (_, index) =>
    ...
    if (freshMerged.length >= this.minFixturesBeforeFallback()) {
      return this.compactFixtures(freshMerged);
    }

    const providerGroups: any[][] = [...freshGroups];

    // Camada principal: melhores dados de agenda/odds com baixo custo.
    for (const currentDate of searchDates) {
    ...
    let providerMerged = this.filterDashboardFixtures(
      this.mergeUniqueFixtures(providerGroups),
    );

    if (providerMerged.length >= this.minFixturesBeforeFallback()) {
      await this.saveFixturesCache(providerMerged);
      return this.compactFixtures(providerMerged);
    }

    // FotMob é valiosa para stats; para listar jogos, só usa quando autorizado ou quando falta volume.
    if (this.shouldUseFotmobForFixtures() || providerMerged.length < this.minFixturesBeforeFallback()) {
      for (const currentDate of searchDates) {
    ...
    providerMerged = this.filterDashboardFixtures(
      this.mergeUniqueFixtures(providerGroups),
    );
    ...
    // SportScore6 retorna agenda global. Chama uma única vez como fallback.
    if (this.shouldUseSportScore6Fallback()) {
    ...
    // Providers extras ficam desligados por padrão para poupar cota.
    if (this.shouldUseSportsDbFallback()) {
      for (const currentDate of searchDates) {
    ...
    if (this.shouldUseSportmonksFallback()) {
      for (const currentDate of searchDates) {
    ...
    if (this.shouldUseFootballDataFallback()) {
      for (const currentDate of searchDates) {
  ...
  async getLiveFixtures() {
    ...
    // Live principal: FlashScore + AllScores. FotMob só quando liberado ou como fallback.
    const flashScore = await this.getLiveFixturesFromFlashScore();
    if (flashScore.ok && flashScore.data.length > 0) pushLive(flashScore.data);

    const allScores = await this.getLiveFixturesFromAllScores(today);
    if (allScores.ok && allScores.data.length > 0) pushLive(allScores.data);

    if (groups.length === 0 || this.shouldUseFotmobForLive()) {
      const fotmob = await this.getLiveFixturesFromFotmob();
      if (fotmob.ok && fotmob.data.length > 0) pushLive(fotmob.data);
    }

    if (groups.length === 0 && this.shouldUseSportScore6Fallback()) {
      const sportScore6 = await this.getLiveFixturesFromSportScore6();
      if (sportScore6.ok && sportScore6.data.length > 0) pushLive(sportScore6.data);
    }

    if (groups.length === 0) {
      const sportScore = await this.getLiveFixturesFromSportScore();
      if (sportScore.ok && sportScore.data.length > 0) pushLive(sportScore.data);
    }

    if (groups.length === 0 && this.shouldUseApiFootballFallback()) {
      const apiFootball = await this.getLiveFixturesFromApiFootball();
      if (apiFootball.ok && apiFootball.data.length > 0) pushLive(apiFootball.data);
    }

    if (groups.length === 0 && this.shouldUseSportmonksFallback()) {
      ...
      if (sportmonksKey) {
        try {
          ...
          pushLive((response.data?.data || []).map((item: any) => this.mapSportmonksFixture(item)));
        } catch {}
      }
    }

    if (groups.length === 0 && this.shouldUseFootballDataFallback()) {
      const footballData = await this.getFixturesFromFootballData(today);
      if (footballData.ok && footballData.data.length > 0) pushLive(footballData.data);
    }
  ...
  async debug(date?: string) {
    ...
    const debugFetchProviders = process.env.ODDIX_DEBUG_FETCH_PROVIDERS === 'true';
    const skippedDebug = { ok: false, data: [], error: 'Debug leve ativo. Use ODDIX_DEBUG_FETCH_PROVIDERS=true para testar providers.' } as any;

    const flashScore = debugFetchProviders ? await this.getFixturesFromFlashScore(date) : skippedDebug;
    const flashScoreLive = debugFetchProviders ? await this.getLiveFixturesFromFlashScore() : skippedDebug;
    const allScores = debugFetchProviders ? await this.getFixturesFromAllScores(date) : skippedDebug;
    const allScoresLive = debugFetchProviders ? await this.getLiveFixturesFromAllScores(date) : skippedDebug;
    const fotmob = debugFetchProviders ? await this.getFixturesFromFotmob(date) : skippedDebug;
    const fotmobLive = debugFetchProviders ? await this.getLiveFixturesFromFotmob() : skippedDebug;
    const sportScore6 = debugFetchProviders ? await this.getFixturesFromSportScore6(date) : skippedDebug;
    const sportScore6Live = debugFetchProviders ? await this.getLiveFixturesFromSportScore6() : skippedDebug;
    const sportScore = debugFetchProviders ? await this.getFixturesFromSportScore(date) : skippedDebug;
    const sportScoreLive = debugFetchProviders ? await this.getLiveFixturesFromSportScore() : skippedDebug;
    const sportsDb = debugFetchProviders ? await this.getFixturesFromSportsDb(date) : skippedDebug;
    const sportmonks = debugFetchProviders ? await this.getFixturesFromSportmonks(date) : skippedDebug;
    const footballData = debugFetchProviders ? await this.getFixturesFromFootballData(date) : skippedDebug;
    ...
    const live = debugFetchProviders ? await this.getLiveFixtures() : [];
    ...
    return {
      ...
      note: 'Modo economia ativo. Agenda: cache > FlashScore > AllScores > FotMob fallback > SportScore6 fallback. Live: cache > FlashScore > AllScores > FotMob fallback. API-Football/SportMonks/FootballData/TheSportsDB só com fallback explícito.',
