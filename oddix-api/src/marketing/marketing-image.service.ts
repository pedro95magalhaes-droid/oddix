...
export class MarketingImageService {
  ...
  async generateSimpleBackground(params: {
    ...
    const prompt = prompts[((params.variation || 1) - 1) % prompts.length];

    return this.generateWithPollinations(prompt, 'simple', 1062, 515);
  }

  async generateMultipleBackground(params?: { variation?: number }): Promise<string | null> {
    if (!this.enabled()) return null;
  ...
  private async generateWithPollinations(
    ...
    try {
      ...
      const url =
        ...
        `?width=${width}&height=${height}&model=flux&nologo=true&enhance=true&private=true&seed=${Date.now()}`;
